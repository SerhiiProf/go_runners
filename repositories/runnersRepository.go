...
func (rr RunnersRepository) CreateRunner(runner *models.Runner) (*models.Runner, *models.ResponseError) {
	...
	var runnerId string
	//*****************
	// я бы такую конструкцию применил вместо for rows.Next(), т.к. явно показывает, что только одна строка в rows
	// if succsess := rows.Next(); succsess {
...
func (rr RunnersRepository) UpdateRunner(runner *models.Runner) *models.ResponseError {
	...
	// автор использует res.RowsAffected() как признак выполнения операции в БД. Хотя в спеке написано:
